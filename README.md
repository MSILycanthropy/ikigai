...
| Monitors | The rail is on every screen. `monitor` in `shell.json` names the primary: toasts, the lock, polkit and welcome cards go there, the greeter's card too (the shell leaves the name in `/var/lib/ikigai/greeter/<user>` for it), and the Alt+Tab switcher. `"taskbar": "screen"` makes each rail show only its own screen's windows, Windows' "where the window is open" option; the default `"all"` shows every window on every rail |
...
| Switcher | Windows-style Alt+Tab in the rail's language: hold Alt, Tab cycles live previews most-recent-first across workspaces (minimized included), release to switch, Shift+Tab backwards, Escape cancels, click a tile. It opens on the primary screen whichever monitor the focused window or the mouse is on, as Windows' does. Previews come from the bridge over ext-image-copy-capture |
...
| Memory | zram: compressed swap the size of RAM, zstd, before any disk swap; `vm.swappiness=100` and no swap readahead, since swap is RAM and the kernel should compress a build's idle pages before it drops a game's mapped files. systemd-oomd kills the one runaway app when the apps' slice has sat under memory pressure, or swap is nearly full, instead of the box thrashing; the shell and compositor are never candidates |
...
| Gaming | Not installed by default. `ikigai-steam` installs Steam, the 32-bit driver for your GPU, gamemode (puts you in its group and seeds `~/.config/gamemode.ini` with `renice=10`, so `gamemoderun %command%` on a game runs it at nice -10 from your next login; gamemode's own default renices nothing), gamescope and mangohud and launches it; Proton comes with Steam, [protonup-qt](https://github.com/DavidoTek/ProtonUp-Qt) for Proton-GE |
