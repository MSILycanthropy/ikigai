...
# scx_loader swaps the CPU scheduler for sched_ext's LAVD (config/system/etc/scx_loader):
# games and the UI preempt a background build instead of sharing the cores with it.
...
# oomd, the scheduler (sched_ext swaps live, every task migrates), zram0 (its generator ran
# with the daemon-reload in configs.sh), plocate's first index.
if ! sudo systemd-detect-virt -rq; then
  sudo systemctl start systemd-oomd
  sudo systemctl start scx_loader || echo "scx_loader failed to start (no sched_ext in this kernel?); EEVDF stays"
