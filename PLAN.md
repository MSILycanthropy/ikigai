...
| Screenshots | `grim` + `slurp` + `satty` (cosmic-comp serves ext-image-copy-capture, never wlr-screencopy: grim ≥ 1.5 works, wf-recorder and flameshot don't). `bin/ikigai-shot region\|screen` on `Print` / `Shift+Print` → `shell/Shot.qml` when the shell offers the `shot` IPC target, else slurp. Picker (2026-09-04): overlays map transparent with a blank cursor, grim freezes every output, the frozen image fades in with a pill: Region / Window / Screen × Snip / Edit / Record. Snip = clipboard + `~/Pictures/Screenshots`; Edit = satty; the crop is a clipped view of the frozen image grabbed at physical size. Open: Window mode (one-shot geometry request to the bridge), Record (gpu-screen-recorder `-w region\|screen`, rail indicator, stop via its IPC socket; VM has no GPU, stub-test only), and cosmic-comp 1.7.0 painting the pointer into output captures with paint-cursors off (grim sends options 0; to check on real hardware). |
