...
| Daemons kept under the target | Done 2026-09-04: `ikigai-settings-daemon.service` and `ikigai-idle.service` wanted by the target. The settings daemon logs "Failed to sync with greeter" (it writes for cosmic-greeter; ours reads nothing there) and stays for brightness-by-Settings, sounds, geoclue theme switching, input sources. cosmic-idle serves org.freedesktop.ScreenSaver and locks via `loginctl lock-session`. cosmic-osd replaced (see Status items). |
...
**H. Greeter + lock.** Both done (Greeter and Lock rows). Left: drop cosmic-greeter/daemon from
the package list at the swap. A first-login welcome card in the shell (keys, the rail, where
settings live) was decided 2026-09-04 in place of cosmic-initial-setup.
